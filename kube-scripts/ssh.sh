...
# kube-scripts/ssh.sh — Shell into a sudo-agent running in Kubernetes
...
if [[ -z "$NAME" ]]; then
  echo "Usage: bash kube-scripts/ssh.sh --name" >&2
  echo "Example: bash kube-scripts/ssh.sh --alice" >&2
  exit 1
fi
if [[ "${NAME,,}" == "all" ]]; then
...
kubectl exec -it "deploy/sudo-$NAME" -- bash
